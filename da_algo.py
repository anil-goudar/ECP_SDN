...
class DA:
    def __init__(
        self, data, max_m, gamma, t_start=None, t_min=1e-5, alpha=0.9, interactive=None
    ):
    ...
    def init_crtl_T(self):
        ...
        C_xy = sum(
            self.p_xy[(i, 0)]
            * (np.reshape(self.x[i], (-1, 1)) - np.reshape(self.y[0], (-1, 1)))
            @ (np.reshape(self.x[i], (-1, 1)) - np.reshape(self.y[0], (-1, 1))).T
    ...
    def update_da(self):
        ...
        self.d_xy = {
            (i, j): self.sed(self.x[i], self.y[j])
            for i in range(self.N)
            for j in range(self.m)
        }
        self.d_yy = {
            (j, j_): self.sed(self.y[j], self.y[j_])
            for j in range(self.m)
            for j_ in range(self.m)
        }
        self.D_xy = {
            (i, j): self.d_xy[(i, j)]
            + self.gamma * sum(self.d_yy[(j, j_)] for j_ in range(self.m))
            for i in range(self.N)
            for j in range(self.m)
        }
        return None
    ...
    def calc_associations(self):
        ...
        self.p_yx = {
            (i, j): exp(reg - self.D_xy[(i, j)] / self.T)
            for i in range(self.N)
            for j in range(self.m)
        }
        self.normalize_p()
        self.p_y = [
            sum(self.p_yx[(i, j)] * self.p_x for i in range(self.N))
            for j in range(self.m)
        ]
        self.p_xy = {
            (i, j): (self.p_yx[(i, j)] * self.p_x) / self.p_y[j]
            for i in range(self.N)
            for j in range(self.m)
        }
        self.C = [
            sum(self.p_xy[(i, j)] * self.x[i] for i in range(self.N))
            for j in range(self.m)
        ]
        ...
        if self.m == 1:
            ...
            for i in range(0, size, self.d):
                for j in range(0, size, self.d):
                    self.theta[i : i + self.d, j : j + self.d] = (
                        self.eta * I if i == j else -self.gamma * I
                    )
        return None
    ...
    def calc_obj(self, idx, ctrds):
        d_xy = {
            (i, j): self.sed(self.x[i], ctrds[j])
            for i in range(self.N)
            for j in range(self.m)
        }
        d_yy = {
            (j, j_): self.sed(ctrds[j], ctrds[j_])
            for j in range(self.m)
            for j_ in range(self.m)
        }
        D_xy = {
            (i, j): d_xy[(i, j)]
            + self.gamma * sum(d_yy[(j, j_)] for j_ in range(self.m))
            for i in range(self.N)
            for j in range(self.m)
        }
        D_hard = sum(
            self.chk(idx[i], j) * D_xy[(i, j)]
            for i in range(self.N)
            for j in range(self.m)
        )
        return D_hard
    ...
    def update_hist(self):
        ...
        self.clusters = [
            [self.x[i] for i in range(self.N) if hard_idx_x[i] == j]
            for j in range(self.m)
        ]
        self.proj_idx = []
        ...
        if (
            len(self.hist_hard) != 0 and self.D_hard < min(self.hist_hard)
        ) or self.ctr == 1:
            self.y_best = self.y.copy()
    ...
    def plt_train(self, dur):
        ...
        plt.title("ECP-LL, gamma={0}, N={1}".format(self.gamma, self.N))
    ...
    def train(self):
        ...
        while self.T > self.T_min:
            ...
            if self.interactive in (1, 2, 3, 4):
                ...
                ****************************
                """.format(
                        self.ctr, self.T, self.D_soft, self.hist_best_hard[-1]
                    )
                )
        ...
        np.save(
            "results.npy",
            {"controller_locs": self.ctrds_best, "clusters": self.clusters_best},
        )
